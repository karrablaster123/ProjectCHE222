...
for i = 1:length(xsol.y(1, :))
    ...
    dTdtdebug(:, i) = ODE2(0, x, p);
end
figure;
ax4 = axes();
hold on;
semilogy(ax4, xsol.y(2, :) - 273.15, dTdt(2, :));
%semilogy(ax4, xsol.y(2, :) - 273.15, dTdtdebug(1,:));
%semilogy(ax4, xsol.y(2, :) - 273.15, dTdtdebug(2,:));
%legend(["Actual" "Debug1" "Debug2"]);
xlim([80 220]);
...
function f = ODE2(~, x, p)
    ...
    f = [(K1*exp(-E_1/(k_B*T))*A_1*(x_f^n));
        (K2*A_2*x_i*(a/a0)*exp(-z/z0)*exp(-E_2/(k_B*T)))];
